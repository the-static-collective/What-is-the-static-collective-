...
- [ ] **Step 6:** Verify all five files exist on `agent/collective-memory-notebook` and contain no claims of private subjective memory.
...
**Interfaces:**
- Produces: evidence that only the approved notebook + witness slice is present in the PR.

- [ ] **Step 1:** Compare `main` with `agent/collective-memory-notebook` and inspect changed paths.
- [ ] **Step 2:** Confirm there are no runtime, dependency, binary, credential, or unrelated changes.
- [ ] **Step 3:** Update PR #1 to mention the `witness/` shelf and its governing rule.
