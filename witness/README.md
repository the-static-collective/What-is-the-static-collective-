...
It exists for things that may help a future session recover useful continuity but do not belong in project authority, the shared glossary, or a mature cross-project pattern.
...
A future assistant session may read these pages when relevant, correct them when better evidence appears, and add a small breadcrumb when something seems worth surviving the conversation.

This is **external memory**, not automatic autobiographical memory. A future session should retrieve these notes before claiming continuity; the presence of a note here does not mean every later session has already seen or remembered it.
...
Keep entries small and label what kind of claim they are.
...
2. distinguish what the note says from what the current evidence says;
3. preserve corrections rather than defending an old breadcrumb;
4. never let this shelf outrank a project's canonical record.

This shelf is allowed to be weird. It is not allowed to be falsely certain.
